...
const Wishlist = () => {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      const response = await fetch(`https://backendcapstone-xxrp.onrender.com/api/wishlist/${user?.user._id}`,
  ...
  const deleteButton = async (e) => {
    ...
    const response = await fetch(`https://backendcapstone-xxrp.onrender.com/api/wishlist/${e.currentTarget.id}`, {
      method: "DELETE",
        ...
        {wishlist.map(item => (
          <div key={item?._id} className="card bg-base-100 outline outline-1 outline-slate-200">
            <figure className="px-10 pt-10">
              <img src={`https://backendcapstone-xxrp.onrender.com/Images/${item?.image}`} alt="Shoes" className="rounded-xl" />
            </figure>
